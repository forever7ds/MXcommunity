...
import com.example.mxcommunity.Utils.CookieUtil;
...
public class LoginTicketInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        System.out.println("cookie inteceptor down !");
        if (ticket != null) {
            // 查询凭证
            OnlineTicket loginTicket = userService.findLoginTicket(ticket);
            // 检查凭证状态（是否有效）以及是否过期
            if (loginTicket != null && loginTicket.getOnlineStatus() == 0 && new Date(System.currentTimeMillis() + loginTicket.getExpiredSeconds()).after(new Date())) {
                // 根据凭证查询用户
                User user = userService.getUserById(loginTicket.getUserId());
                // 在本次请求中持有用户信息
                hostHolder.setUser(user);
                System.out.println("user set down !");
...
//                SecurityContextHolder.setContext(new SecurityContextImpl(authentication));
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        System.out.println("model view interceptor done");
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
        hostHolder.clear();
        System.out.println("end interceptor successful !");
